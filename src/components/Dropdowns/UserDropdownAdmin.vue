...
<div>
    ...
    <div ref="popoverDropdownRef" class="bg-white text-base z-50 float-left py-2 list-none text-left rounded shadow-lg min-w-48" v-bind:class="{
        ...
        <button @click="$router.push('/admin/profile')& close()" class="text-sm py-2 px-4 font-normal block w-full whitespace-no-wrap bg-transparent text-gray-800">
            ข้อมูลส่วนตัว
        </button>
        <button @click="$router.push('/admin/farmer')& close()" class="text-sm py-2 px-4 font-normal block w-full whitespace-no-wrap bg-transparent text-gray-800">
        ...
        <button @click="logout()" class="text-sm py-2 px-4 font-normal block w-full whitespace-no-wrap bg-transparent text-gray-800">
            ออกจากระบบ
        </button>
    </div>
</div>
