<template>
  <div v-if="response" class="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded"
      ...
      <div class="flex flex-wrap items-center">
        ...
        <form @submit.prevent="run">
          <div class="relative flex flex-wrap ">
            <input type="text" v-model="search" placeholder="ค้นหา"
        ...
        <div class="relative w-full px-4 max-w-full flex-grow flex-1 text-right">
          <button @click="$router.push('/admin/addAdmin')"
      ...
      <table class="items-center w-full bg-transparent border-collapse">
        ...
        <tr v-for="admin,i in admins.results" :key="i">
          <th class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-no-wrap p-4 text-left flex items-center">
            <img :src="(admin.profile_image)?admin.profile_image:`https://sv1.picz.in.th/images/2020/11/04/bQMzml.jpg`"
          ...
          <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-no-wrap p-4">
            <div class="flex items-center">
              ...
              <button @click="deleteAdmin(admin.id)" class="bg-red-500
                  active:bg-red-600 font-bold uppercase text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150 f-white "
                      type="button">
                ลบ
              </button>
                ...
                <div class="flex flex-wrap">
                  ...
                  <div class="w-full lg:w-12/12 px-4">
                    <div class="relative w-full mb-3">
                      ...
                      <input required type="text" v-model="form.first_name" :class="$xinput">
                    </div>
                  </div>
                  <div class="w-full lg:w-12/12 px-4">
                    <div class="relative w-full mb-3">
                      ...
                      <input required type="text" v-model="form.last_name" :class="$xinput">
                    </div>
                  </div>
                </div>
                <div class="w-full lg:w-12/12 px-4 mt-2">
                  <div class="relative w-full mb-3">
                    <button :class="`bg-orange-600 f-white ${$btn}`">แก้ไขข้อมูล</button>
                  </div>
                </div>
              </form>
