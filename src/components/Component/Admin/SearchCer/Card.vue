...
<div>
    <div class=" px-4 md:px-10 mx-auto ">
        <form @submit.prevent="run" class=" w-full animate__animated animate__bounceInUp">
            <center>
                <h2 class="text-2xl font-bold text-blue-800">ค้นหาพันธุ์ประวัติ</h2>
            </center><br>
            <input :class="$search1" v-model="search" type="text" placeholder="ค้นหา เช่น ชื่อควาย ชื่อเจ้าของ ชื่อฟาร์ม  กลุ่มเกษตรกร จังหวัด อำเภอ ตำบล ของฟาร์ม" />
        </form>
    </div>

    <!-- card --><br>
    <div class="   px-1 md:px-10 mx-auto w-full    overflow-x-auto animate__animated animate__bounceInLeft animate__delay-1s">
            ...
            <div class="w-full p-3  md:w-56  hvr-grow" v-for="buffalo,index in buffalos.results" :key="index">
                <div class=" w-full bg-white rounded-xl shadow-xl  ">
                    ...
                    <div class="p-5">
                        ...
                        <button :class="`${$btn} f-white rounded p-2 bg-blue-500 hover:bg-blue-700 hover:shadow-lg focus:outline-none focus:ring-4 focus:ring-blue-600 focus:ring-opacity-50 transition duration-500 ease-in-out transform hover:-translate-y-1 hover:scale-110`" @click="$router.push(`buffalo/profile?id=${buffalo.id}`)">
